...
button-gated sequence:
    ...
    2. align XY    - each drone moves to its trajectory start position.
    3. fly         - all drones execute their trajectories simultaneously.
    4. land        - all drones land.

Pre-requisites
--------------
- Crazyswarm2 server running (``ros2 launch crazyflo_planner real.launch.xml``).
- Trajectory CSVs present under data/<MISSION>/traj_cf{1,2,3}.csv.

Usage
-----
    ros2 run crazyflo_planner mission [MISSION]

Available missions: ellipse, figure8, line_course.
...
# Root directory to search for mission data folders
...
def list_missions(data_root: Path) -> list[str]:
    ...
    return sorted(
        ...
        if d.is_dir() and (d / 'traj_cf1.csv').exists()
    )
...
def main():
    ...
    print(f"Loading mission '{MISSION}' from {data_path}")
    ...
    # ------------------------------------------------------------------
    csv_paths = [data_path / f'traj_cf{i+1}.csv' for i in range(3)]
    ...
    # ------------------------------------------------------------------
    # Upload trajectories to crazyflies
