...
if __name__ == "__main__":
    ...
    for p in traj.polynomials:
        ts = np.linspace(0, p.duration, args.num)
    ...
    for i in range(1, len(tss)):
        constraints.append(
            {'type': 'eq', 'fun': func_eq_constraint_val, 'args': (i, tss, yawss)})
        constraints.append(
            {'type': 'eq', 'fun': func_eq_constraint_der, 'args': (i, tss, yawss)})

    # zero derivative at the beginning and end
    constraints.append(
        {'type': 'eq', 'fun': func_eq_constraint_der_value, 'args': (0, tss[0][0], 0)})
    constraints.append(
        {'type': 'eq', 'fun': func_eq_constraint_der_value,
         'args': (len(tss)-1, tss[-1][-1], 0)})

    res = scipy.optimize.minimize(
        func, x0, (tss, yawss), method="SLSQP",
        options={"maxiter": 1000},
        constraints=constraints)
    print(res)

    for i, p in enumerate(traj.polynomials):
        p.pyaw.p = np.array(res.x[i*8:(i+1)*8][::-1])
